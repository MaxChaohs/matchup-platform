...
interface EditPlayerRecruitmentModalProps {
  recruitment: PlayerRecruitment;
  onClose: () => void;
  onSuccess: () => void;
}

export default function EditPlayerRecruitmentModal({ recruitment, onClose, onSuccess }: EditPlayerRecruitmentModalProps) {
  const [formData, setFormData] = useState({
    title: recruitment.title,
    category: recruitment.category,
    region: recruitment.region,
    dayOfWeek: recruitment.dayOfWeek,
    time: recruitment.time,
    location: recruitment.location,
    description: recruitment.description || '',
    neededPlayers: recruitment.neededPlayers,
    currentPlayers: recruitment.currentPlayers,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6 my-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">編輯隊員招募</h2>
